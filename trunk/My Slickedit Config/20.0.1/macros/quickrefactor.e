...
//   }
   ...
   if (new_beautifier_supported_language(lang)) { 
      ...
      add_blank_line = (opts[BL_BETWEEN_METHODS] == '1');
   }
...
int _java_generate_debug(VS_TAG_BROWSE_INFO cm, _str prefix)
{
   ...
   if(cm.type_name == 'proc' || cm.type_name == 'func') {
      ...
      line := indent_one_level :+ 's_log.debug("' :+ cm.member_name :+ '(';
      if(function_info.arguments._length() != 0) {
         for(i:=0; i < function_info.arguments._length(); i++) {
...
int _c_generate_debug(VS_TAG_BROWSE_INFO &cm, _str prefix) 
{
   ...
   if(cm.type_name == 'proc' || cm.type_name == 'func') {
      ...
      if (function_info.arguments._length() != 0) {
         for (i:=0; i < function_info.arguments._length(); i++) {
            ...
            if (function_info.arguments[i].ParamType == "osp_cppUtil::String&" || function_info.arguments[i].ParamType == "osp_dataSource::String&") {
               strappend(line, function_info.arguments[i].ParamName'=');
               strappend(line, '%s');
            } else if (function_info.arguments[i].ParamType == "osp_serverUtil::Key&" || function_info.arguments[i].ParamType == "osp_cppUtil::Key&") {
               strappend(line, function_info.arguments[i].ParamName'=');
               strappend(line, '%s');
            } else {
               strappend(line, function_info.arguments[i].ParamName'=');
      ...
      for (i:=0; i < function_info.arguments._length(); i++) {
         ...
         if (pos('&', function_info.arguments[i].ParamType) > 0) {
            ...
            if (function_info.arguments[i].ParamType == "osp_cppUtil::String&" || function_info.arguments[i].ParamType == "osp_dataSource::String&") {
               ...
               strappend(line, ', ' :+ function_info.arguments[i].ParamName :+ '.getXML().value()');
            } else {
               strappend(line, ', ' :+ '&' :+ function_info.arguments[i].ParamName);
            }
         } else {
      ...
      if (cm.return_type == "osp_cppUtil::String" || cm.return_type == "osp_dataSource::String") {
         ...
         insert_line(line);
         return 0;
      } else if (endsWith(cm.return_type, "Key&") || endsWith(cm.return_type, "Key")) {
         line := indent :+ '_debug.printf("' :+ var_access :+ '=';
         strappend(line, '%s' :+ '\n", ' :+ var_access :+ '.getXML().value());');
         insert_line(line);
         return 0;
      }
      ...
      if(get_var_def(cm, var_def) == 0 && (var_def.type_name == 'struct' || var_def.type_name == 'class')) {
         ...
         for(i:=0; i < member_list._length(); i++) {
            ...
            line := indent :+ '_debug.printf("' :+ member_access :+ '=';
