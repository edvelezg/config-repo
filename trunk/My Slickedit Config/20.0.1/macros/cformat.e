 ...
 *
 */
_command gui_beautify(_str lang='') name_info(','VSARG2_EDITORCTL|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveBeautifiers()) {
   ...
   if (new_beautifier_has_standard_config(lang)) {
      return beautifier_edit_current_profile(lang);
   }
   ...
   if ( !index ) {
      if ( BeautifyCheckSupport(lang) ) {
         ...
         if (new_beautifier_supported_language(lang)) {
            beautifier_edit_current_profile(lang);
            return '';
         }
...
_command int beautify(boolean quiet=false) name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveBeautifiers()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Beautify");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
_command int c_format,c_beautify(int in_wid=0, int start_indent=0, boolean use_form_options=false, boolean quiet=false) name_info(','VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveBeautifiers()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Beautify");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
 ...
 */
_command int c_format_selection,c_beautify_selection(boolean use_form_options=false, boolean quiet=false) name_info(','VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveBeautifiers()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Beautify");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
{
   return _getSysconfigMaybeFixPath("formatter":+FILESEP:+FORMAT_INI_FILENAME);
}
...
#define DEFAULT_BEAUTIFIER_EXT  "xhtml=xml phpscript=c ada=ada c=c java=c js=c cs=c e=c html=html cfml=html xml=html xsd=html vpj=xml docbook=xml ant=xml as=c d=c android=xml ruby=ruby"   /* Valid beautifier languages */
...
#define MSG_BEAUTIFY 'Beautifies the buffer with the default profile for the language'
#define MSG_EDIT_PROFILE 'Edits the settings for the current profile'
#define MSG_BEAUTIFIER_OPTIONS 'Allows you to edit or change the default beautifier profile.'
...
void _init_menu_nbeautifier(int menu_handle, int no_child_windows)
{
   if (_haveBeautifiers()) {
      ...
      for (i=0;i<Nofitems;++i) {
         ...
         _str nohotkey_caption=stranslate(caption,"","&");
   ...
   boolean both_standard = new_beautifier_has_standard_config(lang) && new_beautifier_has_standard_config(otherlang);
   boolean html_mix = otherlang != lang;
   ...
   if (handled_by_new) {
      _menu_insert(ep_handle, -1, 0, "Beautify ("LanguageSettings.getBeautifierProfileName(lang)')', 'beautify', '', '', 
                   MSG_BEAUTIFY);
      ...
      if (html_mix) {
         ...
         clo := _LangId2Modename(cursor_lang);
         ...
         mno := _LangId2Modename(otherlang);
