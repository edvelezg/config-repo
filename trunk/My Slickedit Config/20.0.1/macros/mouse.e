...
*/
static int _adjusted_windent_x() {
...
static boolean selectNscroll(_str &mark_name,_str mstyle,boolean lastkey_not_rbutton,int mx,int my,int wx,int wy,boolean select_words,
...
static int _mou_click_handle_drag_drop(int mx,int my,int wx,int wy,_str mark_name, _str mstyle, boolean select_words, _str menu_option,int mark_locked,int first_word_start_offset,int first_word_end_offset, boolean lastkey_not_rbutton, typeless orig_pos,_str old_scroll_style,_str copytext_option) {
   ...
   for (;;) {
       ...
       if ( event:==MOUSE_MOVE ) {
          ...
          if ( mou_in_window3(mx,my) ) {
             ...
             boolean done=selectNscroll(mark_name,mstyle,lastkey_not_rbutton,mx,my,wx,wy,select_words,first_word_start_offset,first_word_end_offset);
             if ( done ) {
                break;
             }
          }
...
{
   _str ename=event2name(last_event());
   int add_cursor;
   // Hex mode does not support multiple cursors
   if (p_hex_mode || _isdiffed(p_buf_id)) {
      arg_add_cursor=0;
   }
   boolean allow_url_click= ename:=='C-LBUTTON-DOWN' || ename:=='M-LBUTTON-DOWN';
   if (allow_url_click && overURL) {
      arg_add_cursor=0;
   }
   ...
   //boolean was_in_selection;
   boolean pending_deselect=false;
   if ( extend_mark_option ) {
      if (p_scroll_left_edge>=0) {
...
_command int add_multiple_cursors(_str mark_name="") name_info(','VSARG2_MARK|VSARG2_TEXT_BOX|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_ICON|VSARG2_NOEXIT_SCROLL)
{
   ...
   if (p_LastModified != gLastModified) {
 ...
 * 
 */
_command void mou_select_word_add_cursor() name_info(','VSARG2_READ_ONLY|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL) {
   mou_select_word(1);
}
/**
 * Selects the current word with a character type selection.
 ...
 * 
 */
_command void mou_select_word(_str arg_add_cursor='',...) name_info(','VSARG2_READ_ONLY|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
         ...
         _str cmd;
          ...
          if (cmd!='') {
             ...
             if (isinteger(arg2)) {
                ...
                if (!doDelete) {
                   doDelete = !_haveProMacros() && (flags & VSARG2_REQUIRES_PRO_EDITION);
                }
             }
#endif
...
void ctlminihtml1.on_create(_str msg='',int x=0, int y=0)
{
   ...
   // double default form height if we have crazy amounts of space
   larger_height := _dy2ly(SM_TWIP,screen_height) - _dy2ly(SM_TWIP,y);
   if (larger_height > 4*p_height) {
      p_height = 2*p_height;
   }
   p_text=msg;
   ...
   p_active_form.p_height=ctlpicture1.p_height+fh;
   ...
   if (_isEditorCtl(false)
      ...
      if (mou_last_x()<_adjusted_windent_x() &&
         ...
         if (lineflags & PLUSBITMAP_LF) {
            ...
            first_non_blank();
            right();
            collapsed_block_msg := "Expand code block";
            comment_msg := "";
            comment_flags := 0;
            if (_in_comment()) {
               _clex_skip_blanks();
               ...
               if (context_id > 0) {
                  ...
                  if (cur_line == p_RLine) {
                     _ExtractTagComments2(comment_flags, comment_msg, 500, cur_tag_name, p_buf_name, cur_line);
                     if (comment_msg != "") {
                        _make_html_comments(comment_msg,comment_flags,"",cur_tag_name,true);
                        collapsed_block_msg = "Expand comment block<hr>":+comment_msg;
                     }
                  }
               } 
            }
            if (comment_msg == "") {
               ...
               collapsed_block_msg :+= "<hr style=\"margin-bottom:0;margin-top:1\">";
               _GetHTMLColorCoding(p_window_id, first_line, num_lines, auto html_msg);
               collapsed_block_msg :+= html_msg;
               if (last_line-first_line > 500) {
                  collapsed_block_msg :+= "<font color=\"black\" style=\"background-color: white\"><b>Remaining "(last_line-first_line-499)" lines truncated.</b></font>";
               }
            }
            if (result=='') {
               result = collapsed_block_msg;
            } else {
               result=result:+"<br>":+collapsed_block_msg;
            }
            restore_pos(before_comment_test);
         } else if (lineflags & MINUSBITMAP_LF) {
            if (result=='') {
               result = "Collapse code block";
            } else {
               result=result:+"<br>":+"Collapse code block";
            }
         }
...
_command void test3()
{
   ...
   _str msg;
   gColorIndex=_AllocColor(0xffffff,0xff);
   msg=substr('',1,10000,'x');
   //msg="line 1<br>line2";
   //int gStreamMarkerIndex=_LineMarkerAdd(p_window_id,p_line,0,4,find_index('_edplus.ico',PICTURE_TYPE),type,msg);
   int i;
   int width=3;
   for (i=0;i<1;++i) {
      gStreamMarkerIndex=_StreamMarkerAdd(p_window_id,_nrseek(),width,false,find_index('_edplus.ico',PICTURE_TYPE),type,"this is a test");
      //_StreamMarkerSetStyleColor(gStreamMarkerIndex,0xff);
      _StreamMarkerSetTextColor(gStreamMarkerIndex,gColorIndex);
      p_col+=width+2;
   }
   //gStreamMarkerIndex=_LineMarkerAdd(p_window_id,p_line,false,0,find_index('_edplus.ico',PICTURE_TYPE),type,"this is a test");
