 ...
 */
_command int refactor_quick_rename(_str precise='') name_info(FILE_ARG','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveRefactoring()) {
   ...
   if (!def_disable_cpp_refactoring && _haveRefactoring() &&
       precise=="precise" && _LanguageInheritsFrom('c', cm.language)) {
...
_command int refactor_fast_rename(_str precise='') name_info(FILE_ARG','VSARG2_REQUIRES_PRO_EDITION)
{
...
VS_TAG_BROWSE_INFO find_proto(VS_TAG_BROWSE_INFO function_cm)
{
   ...
   VS_TAG_BROWSE_INFO nearest_match;
   nearest_match_path := "";
   ...
   for (i=1; i<=num_matches; i++) {
      ...
      if (match.type_name == 'proto') {
         // same file, good enough for me
         //say("find_proto: proto, file_name="match.file_name" nearest="nearest_match_path);
         if (file_eq(match.file_name, function_cm.file_name)) {
            nearest_match = match;
            nearest_match_path = match.file_name;
            break;
         }
         // check for same directory, we could still find a same file match though
         funct_dir := _strip_filename(function_cm.file_name,'N');
         match_dir := _strip_filename(match.file_name,'N');
         if (file_eq(funct_dir, match_dir)) {
            nearest_match = match;
            nearest_match_path = match_dir;
         } else {
            // not the same directory, so check for the most similar directory
            matching_dir_parts := "";
            while (match_dir != "" && funct_dir != "") {
               parse match_dir with auto match_part FILESEP match_dir;
               parse funct_dir with auto funct_part FILESEP funct_dir;
               if (!file_eq(match_dir, funct_dir)) break;
               matching_dir_parts = matching_dir_parts :+ match_part :+ FILESEP;
            }
            if (nearest_match_path == "" || length(matching_dir_parts) > length(nearest_match_path)) {
               nearest_match = match;
               nearest_match_path = match_dir;
            }
         }
 ...
 * @categories Refactoring_Functions
 */
_command int refactor_quick_extract_method(_str precise='') name_info(FILE_ARG','VSARG2_REQUIRES_PRO_EDITION)
{
   // init refactoring operations
   if (!refactor_init(true, false)) {
      return COMMAND_CANCELLED_RC;
...
//   }
   ...
   // Check if they want a blank line between functions
   add_blank_line := true;
   if (new_beautifier_supported_language(lang)) {
      ibeautifier := _beautifier_cache_get(lang);
      add_blank_line = (_beautifier_get_property(ibeautifier,VSCFGP_BEAUTIFIER_BL_BETWEEN_FUNS)>0);
   }

   // Add a blank line before the function if there isn't one already
   if (add_blank_line && !up()) {
      get_line(auto line_before);
      if (line_before != "") {
         insert_line("");
      }
   }
   ...
   gen_index := _FindLanguageCallbackIndex('_%s_generate_function');
   if (gen_index != 0) {
      inside_function_pos=call_index(new_function, 0 , null, extracted_code, 
                                     function_indention, p_SyntaxIndent,
                                     false, false, '', '', gen_index);
   }

   // Add a blank line after the function if there isn't one already
   if (add_blank_line && !down()) {
      get_line(auto line_after);
      up();
      if (line_after != "") {
         insert_line("");
      }
   ...
   if (create_comment == "1") {
      ...
      if (expand_alias('/**', '', getDocAliasProfileName(p_LangId), true)) {
         javadoc_comment();
      }
      commentwrap_SetNewJavadocState();
...
_command int refactor_quick_encapsulate_field() name_info(FILE_ARG','VSARG2_REQUIRES_PRO_EDITION)
{
   // init refactoring operations
   if (!refactor_init(true, false)) {
      return COMMAND_CANCELLED_RC;
                                  ...
                                  VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   while (result == 0) {
      ...
      if ((parens==0 && current_char==")") || (parens==1 && current_char==",")) {
         ...
         _str errorArgs[];
         int status = _c_get_type_of_prefix(errorArgs, argument.text, argument.type, visited, depth+1);
         if (status != 0) {
...
{
   say('get_function_info(function_info='(function_info!=null?'nonnull':'null')' cm='(cm!=null?'nonnull':'null')' )');
   int current_offset = (int)_QROffset();
                                                  ...
                                                  VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   if (p_buf_id == last_bufid && orig_seekpos == last_offset && p_LastModified == last_modified) {
      ...
      get_function_call_info(args,visited,depth+1);
      last_bufid= p_buf_id;
 ...
 * @return int
 */
_command int refactor_quick_modify_params() name_info(FILE_ARG','VSARG2_REQUIRES_PRO_EDITION)
{
   // init refactoring operations
   if (!refactor_init(true, false)) {
      return COMMAND_CANCELLED_RC;
 ...
 */
_command int refactor_quick_replace_literal() name_info(FILE_ARG','VSARG2_REQUIRES_PRO_EDITION)
{
   // init refactoring operations
   if (!refactor_init(true, false)) {
      return COMMAND_CANCELLED_RC;
...
static _str get_printf_type(_str var_type)
{
   ...
   if (pos("char",var_type) != 0 && 
      ...
      if (_c_builtin_assignment_compatible("int", var_type, false)) {
         ...
         type_string = "%lu";
      } else if (_c_builtin_assignment_compatible("unsigned char", var_type, false)) {
         type_string = "0x%02X";
      }
   }
   return type_string;
...
static boolean is_declared_in_current_context(_str name, _str type_name)
{
   ...
   for (i=1; i<=n; i++) {
      ...
      if (!tag_is_local_in_scope(i, seekpos)) continue;
      if (cm.flags & VS_TAGFLAG_ignore) continue;
...
{
   say('_c_generate_debug(cm='(cm!=null?'nonnull':'null')' prefix='prefix' )');
   VS_TAG_BROWSE_INFO var_def;
   tag_browse_info_init(var_def);
   _str indent             = get_line_leading_whitespace();
   _str indent_one_level   = indent :+ indent_string(p_SyntaxIndent);
   _str current_indent = indent;
   _str dbg_print_fn = _LangGetProperty(p_LangId,VSLANGPROPNAME_DEBUG_PRINT_FUNCTION);
   if (dbg_print_fn == null || dbg_print_fn == "") dbg_print_fn = "printf";

   say('cm.type_name='cm.type_name);
   if (cm.type_name == 'proc' || cm.type_name == 'func') {

      VS_TAG_FUNCTION_INFO function_info;
      get_function_info(function_info, cm);

      // Paranoia: ensure we get the right indent level no matter whether
      // the cursor is on the function declaration itself, of if it's on 
      // another reference to it.
      _GoToROffset(cm.name_seekpos);
      indent = get_line_leading_whitespace() :+ indent_string(p_SyntaxIndent);

      // Go to start of function body.
      _GoToROffset(cm.scope_seekpos);

      line := indent :+ dbg_print_fn :+ '("' :+ cm.member_name :+ '(';
      say('cm.member_name='cm.member_name);
      if (function_info.arguments._length() != 0) {
         for (i:=0; i < function_info.arguments._length(); i++) {
      ...
      for (i:=0; i < function_info.arguments._length(); i++) {
         ...
         if (pos('void',function_info.arguments[i].ParamType) > 0) {
            // C function prototype with a 'void' param
         } else if (pos('&', function_info.arguments[i].ParamType) > 0) {
      ...
      _str var_access = prefix :+ cm.member_name;   // String used to access the var in the outputted code
      say('var_access='var_access);
      say('cm.return_type='cm.return_type);
      ...
      if (get_var_def(cm, var_def) == 0 && (var_def.type_name == 'struct' || var_def.type_name == 'class')) {
         tag_browse_info_dump(var_def,'var_def');
         ...
         for (i:=0; i < member_list._length(); i++) {
            ...
            line := indent :+ dbg_print_fn :+ '("' :+ member_access :+ '=';
         ...
         line := indent :+ dbg_print_fn :+ '("' :+ var_access :+ '=';
         say('line='line);
 ...
 */
_command void generate_debug() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveRefactoring()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Refactoring");
      return;
...
/** 
 * Create multiple cursors for all instances of a given symbol 
 * in the current file.
 *  
 ...
 */ 
_command int add_cursors_for_symbol() name_info(','VSARG2_CMDLINE|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY|VSARG2_REQUIRES_PRO_EDITION)
{
   // Disable this feature for the Standard Edition
   if (!_haveContextTagging()) {
   ...
   VS_TAG_IDEXP_INFO idexp_info;
   VS_TAG_RETURN_TYPE visited:[];
   status := _Embeddedget_expression_info(false, auto lang, idexp_info, visited);
   if (status < 0 || idexp_info.lastid == "") {
      message("Could not identify symbol under cursor: ":+get_message(status));
      return status;
   }

   // get browse information for the tag under the symbol
   struct VS_TAG_BROWSE_INFO cm;
   status = tag_get_browse_info("", cm, false, null, false, true, true, false, false, false, visited);
   if (status == COMMAND_CANCELLED_RC) return status;
   if (status < 0 || cm.member_name=="") {
      message("Could not identify symbol: ":+get_message(status));
      return status;
   }
   ...
   if (context_id > 0 && (cm.type_name=='lvar' || cm.type_name=='param')) {
   ...
   int maxReferences = def_cb_max_references;
   int numReferences = 0;
   tag_match_occurrences_in_file_get_positions(auto errorArgs, seekPositions,
                                               cm.member_name, p_EmbeddedCaseSensitive,
                                               cm.file_name, cm.line_no,
                                               VS_TAGFILTER_ANYTHING, 
   ...
   if (seekPositions._length() <= 1) {
      message("The symbol under the cursor has only one reference.");
      return 0;
   }

   // Set all the cursors at the position within the symbol where cursor was
   orig_col   := p_col;
   col_offset := (orig_col - idexp_info.lastidstart_col);
   _MultiCursorClearAll();
   for (j:=0; j<seekPositions._length(); j++) {
      // go to the seek position
      status = _GoToROffset(seekPositions[j]+col_offset);
      if (status < 0) continue;
      _MultiCursorAdd();
   }
   ...
   // That's all folks
   restore_pos(p);
   message(nls("Added %s cursors for symbol '%s'",seekPositions._length(),cm.member_name));
   return(0);
}

int _OnUpdate_add_cursors_for_symbol(CMDUI &cmdui,int target_wid,_str command)
{
   return(_OnUpdate_push_ref(cmdui,target_wid,command));
}
