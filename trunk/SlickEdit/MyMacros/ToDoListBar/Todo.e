 ...
 *  
 * Changelog: 
 * 	1.0	Work around SlickEdit RegEx bug. 
 *  
 * 	0.9	Support for new tool window infrastructure. 
 *  
 *      0.8     Be less picky about how a user starts their ToDo
 *      	descriptions. Don't require alphanumeric.
 *  
 ...
 */
const TODO_TDML_REGEX = '{#0TODO}[ \t]*{#1[\!\^]:*0,1}[ \t]@{#2:a[~\:]@}*\:';
/*
 * TDML_ALT_REGEX: 
 *   :b* 		- Maximal match of 0 or more spaces and tabs
 *   {#3:a?@}*		- Comment
...
static void RenderTodo()
{
    mou_hour_glass(true);
    #if __VERSION__ >= 20
    int tree = tw_is_visible(_TBTODO).p_child;
    #else
 ...
 */
static void init_toolwindow()
{
    #if __VERSION__ >= 20
	int formid = tw_find_form(_TBTODO);
	if (!formid)
	{
	    int sts = tw_register_form(_TBTODO, 0, DOCKAREAPOS_NONE, 0);
	    if (sts != 0)
	    {
		return;
	    }
