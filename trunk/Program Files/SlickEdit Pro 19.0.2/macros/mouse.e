...
static int _mou_click_handle_drag_drop(int mx,int my,int wx,int wy,_str mark_name, _str mstyle, boolean select_words, _str menu_option,int mark_locked,int first_word_start_offset,int first_word_end_offset, int orig_col, boolean lastkey_not_rbutton, typeless orig_pos,_str old_scroll_style,_str copytext_option) {
                       ...
                       int arg_add_cursor=0) name_info(','VSARG2_MARK|VSARG2_TEXT_BOX|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_ICON|VSARG2_NOEXIT_SCROLL)
{
   ...
   if (!select_words && def_dragdrop && !in_left_margin && p_hex_mode!=HM_HEX_ON && !extend_mark_option && !menu_option &&
      ...
      status:=_mou_click_handle_drag_drop(mx,my,wx,wy,mark_name,mstyle,select_words,menu_option,mark_locked,first_word_start_offset,first_word_end_offset,orig_col,lastkey_not_rbutton,orig_pos,old_scroll_style,copytext_option);
