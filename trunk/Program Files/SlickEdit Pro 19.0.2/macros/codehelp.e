...
int def_background_tagging_maximum_kbytes=10000;
...
#define DD_STYLE                 ' style="margin-left:13pt"'
#define DD_STYLE_PARAM           ' style="margin-left:26pt;text-indent:-13pt"'
#define P_STYLE                  ' style="margin-top:0pt;margin-bottom:0pt;" class="JavadocHeading"'
#define BLOCKSTYLE               ' style="margin-top:0pt;margin-bottom:0pt;"'
#define HANGING_INDENT           '-13pt'
#define HANGING_INDENT2          '-26pt'
#define START_DESCRIPTION_TAGS   '<P style="margin-top:0pt;margin-bottom:0pt;" class="JavadocDescription">'
#define START_HEADER_TAGS        '<blockquote style="margin-top:0pt;margin-bottom:0pt;"><p style="text-indent:'HANGING_INDENT';margin-top:0pt;">'
#define START_HEADER_TAGS4       '<blockquote style="margin-top:0pt;margin-bottom:0pt;"><p style="text-indent:'HANGING_INDENT2';margin-top:0pt;">'
#define START_HEADER_TAGS2       '<blockquote style="margin-top:0pt;margin-bottom:0pt;"><p style="margin-top:0pt;margin-left:'HANGING_INDENT';">'
#define START_HEADER_TAGS3       '<blockquote style="margin-top:0pt;margin-bottom:0pt;"><p style="margin-top:0pt;margin-left:'HANGING_INDENT';">'
#define END_HEADER_TAGS            '</blockquote>'
...
_command void auto_functionhelp_key() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   ...
   if(_LanguageInheritsFrom("java") && def_jrefactor_auto_import==1) {
...
static void get_line_comment(int &comment_flags,
                             _str tag_type,
                             _str &comments,
                             int &first_line,
                             int &last_line,
   ...
   _str &doxygen_comment_start)
{
   ...
   boolean hit_3slashes_already=false;
   ...
   for (i=0;i<Noflines;++i) {
      ...
      if (substr(strip(line,'L'),1,3)=='///' && allow_xmldoc_comment && !hit_3slashes_already) {
         hit_3slashes_already = 1;
         ...
         if (rest!='' && 
            ...
            if (first_non_blank_ch=='<') {
               //say("_parse_multiline_comments: XMLDOC");
      ...
      /*} else if(substr(strip(line,'L'),1,3)=='///' && substr(strip(line,'L'),4,1)!='/' && allow_xmldoc_comment) {
         ...
         int oemCallbackIndex = find_index("oem_check_doccomment_prefix",PROC_TYPE);
         if ( oemCallbackIndex && index_callable(oemCallbackIndex) ) {
            call_index(line,hit_3slashes_already,comment_flags,oemCallbackIndex);
         }
      }
      prefix='';
      int j=0;
...
*/
int _GetCurrentCommentInfo(int &comment_flags,
                           _str &orig_comment,
                           _str &return_type, 
                           _str &line_prefix, 
                           int (&blanks):[][],
                           _str &doxygen_comment_start)
{
   ...
   if (!_do_default_get_tag_header_comments(first_line, last_line)) {
      ...
      p_buf_name="";
      _do_default_get_tag_comments(comment_flags, type_name, orig_comment, 
...
void _do_complete()
{
   ...
   int i, num_matches = context_match_tags(errorArgs,lastid,false,def_tag_max_find_context_tags,false,caseSensitive,visited);
   ...
   if (num_matches >= def_tag_max_find_context_tags || timedOut) {
      ...
      _macro_delete_line();
      _do_list_members(false,true,null,null,null,null,true,true,false,visited);
      return;
   }
   ...
   */
      ...
      */
      ...
      if (hashtab._indexin(tag)) {
         member_msg=member_msg:+"<P"P_STYLE"><B>Deprecated.</B>&nbsp;<I>":+hashtab:[tag][0]:+"</i>":+'<br>':+'<br>';
         hashtab._deleteel(tag);
      }
      _str dtstart='<P'P_STYLE'>';
      member_msg=member_msg:+START_DESCRIPTION_TAGS:+description;
      tag="param";
      _str argName='';
      _str text='';
      _str argNameOnly='';
      ...
      if (hashtab._indexin(tag)) {
         ...
         for (i=0;i<count;++i) {
            ...
            if (param_name!='' && argNameOnly==param_name) {
               ...
               member_msg=member_msg:+START_HEADER_TAGS4:+arrowPtr:+"<code><b>":+argAnchor"</b></code> - ":+text:+END_HEADER_TAGS;
            } else {
               member_msg=member_msg:+START_HEADER_TAGS"<code>":+argAnchor:+"</code> - ":+text:+END_HEADER_TAGS;
            }
         }
         hashtab._deleteel(tag);
      }
      ...
      if (hashtab._indexin(tag)) {
         if (return_type!='') {
            member_msg=member_msg:+dtstart"<B>Returns:</B> <B><code>"return_type"</code></b>"START_HEADER_TAGS2:+hashtab:[tag][0]:+END_HEADER_TAGS;
         } else {
            member_msg=member_msg:+dtstart"<B>Returns:</B>"START_HEADER_TAGS2:+hashtab:[tag][0]:+END_HEADER_TAGS;
         }
         hashtab._deleteel(tag);
      }
      tag="exception";
      if (hashtab._indexin(tag)) {
         ...
         for (i=0;i<count;++i) {
            ...
            parse argName with argName '[ \n]','r';
            member_msg=member_msg:+START_HEADER_TAGS:+'<code>'argName'<code>':+" - ":+text:+END_HEADER_TAGS;
         }
         hashtab._deleteel(tag);
      }
      tag="since";
      if (hashtab._indexin(tag)) {
         member_msg=member_msg:+dtstart"<B>Since:</B>"START_HEADER_TAGS2:+hashtab:[tag][0]:+END_HEADER_TAGS;
         hashtab._deleteel(tag);
      }
      tag="serial";
      if (hashtab._indexin(tag)) {
         ...
         count=hashtab:[tag]._length();
         for (i=0;i<count;++i) {
            member_msg=member_msg:+START_HEADER_TAGS3:+hashtab:[tag][i]:+END_HEADER_TAGS;
         }
         hashtab._deleteel(tag);
      }
      tag="serialfield";
      if (hashtab._indexin(tag)) {
         ...
         for (i=0;i<count;++i) {
            ...
            member_msg=member_msg:+START_HEADER_TAGS2:+fieldName' 'fieldType:+" - ":+text:+END_HEADER_TAGS;
         }
         hashtab._deleteel(tag);
      }
      tag="serialdata";
      if (hashtab._indexin(tag)) {
         ...
         count=hashtab:[tag]._length();
         for (i=0;i<count;++i) {
            member_msg=member_msg:+START_HEADER_TAGS2:+hashtab:[tag][i]:+END_HEADER_TAGS;
         }
         hashtab._deleteel(tag);
      }
      tag="author";
      if (hashtab._indexin(tag)) {
         member_msg=member_msg:+dtstart"<B>Author:</B>":+START_HEADER_TAGS2;
         count=hashtab:[tag]._length();
         for (i=0;i<count;++i) {
            if (i==0) {
         ...
         member_msg=member_msg:+END_HEADER_TAGS;
         hashtab._deleteel(tag);
      }
      tag="see";
      _str see_msg="";
      if (hashtab._indexin(tag)) {
         see_msg=see_msg:+dtstart"<B>See Also:</B>":+START_HEADER_TAGS2;
         count=hashtab:[tag]._length();
         for (i=0;i<count;++i) {
         ...
         see_msg=see_msg:+END_HEADER_TAGS;
         hashtab._deleteel(tag);
      }
      _str category_msg="";
      if (make_categories_links) {
         ...
         if (hashtab._indexin(tag)) {
            ...
            if (count>1) {
               category_msg=dtstart"<B>Categories:</B>":+START_HEADER_TAGS2;
            } else {
               category_msg=dtstart"<B>Category:</B>":+START_HEADER_TAGS2;
            }
            for (i=0;i<count;++i) {
               _str category=categories_array[i];
               category=stranslate(category,"",'[\r\n]','r');
               //category='<a href="':+category:+'.html">':+stranslate(category,' ','_'):+'</a>';
               category='<a href="':+'help://':+category:+'">':+stranslate(category,' ','_'):+'</a>';
               if (i==0) {
            ...
            category_msg=category_msg:+END_HEADER_TAGS;
            hashtab._deleteel(tag);
         }
      }
      int j;
