...
void wordwrap_rubout(...)
{
   ...
   if ( def_pull || _insert_state() ) {
      if ( def_hack_tabs && p_col!=1 ) {
         ...
         if (/*def_keys=='brief-keys' && */p_index && p_indent_style==INDENT_SMART && 
                        ...
                        boolean force_select_mode_cb=false)
{
   ...
   */
   ...
   if (lang == '') {
      ...
      if (!setup_index) {
         check_and_load_support(orig_buf_ext,setup_index,buf_name,false,lang);
         lang=_Filename2LangId(buf_name);
      }
   }
   ...
   setup_index = find_index('def-language-'lang, MISC_TYPE);
   if (!setup_index) {
      check_and_load_support(lang,setup_index,buf_name,false,lang);
      setup_index = find_index('def-language-'lang, MISC_TYPE);
   }
   ...
   if ( setup_index ) {
      ...
      if ( !bypass_buffer_setup ) {
         ...
         if (p_SoftWrap && p_buf_size>def_use_softwrap_ksize*1024) {
            ...
            large_file_editing_msg:+="Soft wrap turned off for better performance";
            p_SoftWrap=0;
         }
         ...
         if ( ! read_format_line() ) {
            ...
            p_AutoLeftMargin=langOptions.AutoLeftMargin;
            p_FixedWidthRightMargin=langOptions.FixedWidthRightMargin;
...
{
   VS_LANGUAGE_OPTIONS langOpts;
   if (!_GetDefaultLanguageOptions(lang, lang, langOpts)) {
      if (iwt != '') {
         langOpts.IndentWithTabs = (iwt != 0);
      }
      if (tabs != '') {
         langOpts.szTabs = tabs;
      }
      _SetDefaultLanguageOptions(lang, lang, langOpts);
   }
}
...
{
   // don't try to do this when building state file
 ...
 * @return boolean
 */
static boolean associated_file_exists(_str &filename, _str ext_list, boolean returnAll=false)
{
   mou_hour_glass(true);
...
_str associated_file_for(_str filename, boolean returnAll=false)
{
   ...
   if( file_eq(extension, 'cs') ) {
      ...
      if (strieq(designerExt, "designer")) {
         if (!associated_file_exists(filename, "resx cs", returnAll)) {
            filename = "";
         }
      } else if (strieq(designerExt, "xaml")) {
         if (!associated_file_exists(filename, "xaml cs", returnAll)) {
            filename = "";
         }
      } else {
         if (!associated_file_exists(filename, "designer.cs xaml.cs xaml resx", returnAll)) {
            filename = "";
         }
      }
   ...
   // Visual Studio resource file
      ...
      if (!associated_file_exists(filename, "cs designer.cs vb designer.vb cpp h jsl xaml", returnAll)) {
         filename = "";
      }
   ...
   // Visual Studio xaml (windows presentation foundation) specification file
      ...
      if (!associated_file_exists(filename, "cs xaml.cs vb cpp h jsl", returnAll)) {
         filename = "";
      }
   ...
   // Visual Basic .NET
      ...
      _str designerExt = _get_extension(filename);
      if (strieq(designerExt, "designer")) {
         if (!associated_file_exists(filename, "resx vb", returnAll)) {
            filename = "";
         }
      } else if (strieq(designerExt, "xaml")) {
         if (!associated_file_exists(filename, "xaml vb", returnAll)) {
            filename = "";
         }
      } else {
         if (!associated_file_exists(filename, "designer.vb xaml.vb xaml resx", returnAll)) {
            filename = "";
         }
      }
   ...
   // J#
      ...
      if (!associated_file_exists(filename, "resx xaml", returnAll)) {
         filename = "";
      }
   ...
   // inl -> h, hpp, hp, hh, hxx
      ...
      if (!associated_file_exists(filename, "h hpp hp hh hxx resx xaml qth", returnAll)) {
         filename = "";
      }
      ...
      if (!associated_file_exists(filename, "c m mm resx xaml cpp cp cc cxx c++ inl", returnAll)) {
         filename = "";
      }

   } else if( file_eq(extension, 'qth') ) {

      if (!associated_file_exists(filename, "c cpp cxx c++ h", returnAll)) {
         filename = "";
      }
   ...
   // Ada
      ...
      if (!associated_file_exists(filename, "adb ada", returnAll)) {
         filename = "";
      }

   } else if( file_eq(extension, 'adb') ) {

      if (!associated_file_exists(filename, "ads ada", returnAll)) {
         filename = "";
      }
   ...
   // DigitalMars D
      ...
      if (!associated_file_exists(filename, "di", returnAll)) {
         filename = "";
      }

   } else if( file_eq(extension, 'di') ) {

      if (!associated_file_exists(filename, "d", returnAll)) {
         filename = "";
      }
   ...
   // Slick-C header file
      ...
      // check for source file
      if (!associated_file_exists(filename, "e", returnAll)) {
         filename = "";
      }
      ...
      if (!associated_file_exists(filename, "sh", returnAll)) {
         filename = "";
      }
      ...
      if (!associated_file_exists(filename, "svh svi", returnAll)) {
         filename = "";
      }

   } else if(file_eq(extension,'svi') ) {

      if (!associated_file_exists(filename, "sv svh", returnAll)) {
         filename = "";
      }

   } else if(file_eq(extension,'svh') ) {

      if (!associated_file_exists(filename, "sv svi", returnAll)) {
         filename = "";
      }
      ...
      if (!associated_file_exists(filename, "vrh vri", returnAll)) {
         filename = "";
      }

   } else if(file_eq(extension,'vri') ) {

      if (!associated_file_exists(filename, "vr vrh", returnAll)) {
         filename = "";
      }

   } else if(file_eq(extension,'vrh') ) {

      if (!associated_file_exists(filename, "vr vri", returnAll)) {
         filename = "";
      }

   } else if(file_eq(extension,'rb') ) {
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods, Selection_Functions
 */
_command void copy_lines_up() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_READ_ONLY|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   if (select_active()) {
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods, Selection_Functions
 */
_command void copy_lines_down(boolean do_smart_paste=true) name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_READ_ONLY|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   if (select_active()) {
      markid := _duplicate_selection('');
      if (_select_type(markid) != "LINE") {
